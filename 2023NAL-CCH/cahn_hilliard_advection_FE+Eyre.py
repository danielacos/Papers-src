...
mass_lumping = 0  # Indicates if w is mass-lumped or not
...
P1 = FiniteElement("Lagrange", mesh.ufl_cell(), deg)  # Space of polynomials
...
if spinoidal_decomposition:
    ...
    init_u = Init_u(degree=deg)  # Random data between 0.49 and 0.51
else:
    if adv>0.0:
        init_u = Expression("1.0/2.0*(tanh((0.2-sqrt(pow(x[0]+0.2,2)+pow(x[1],2)))/(sqrt(2.0)*eps))+1.0) + 1.0/2.0*(tanh((0.2-sqrt(pow(x[0]-0.2,2)+pow(x[1],2)))/(sqrt(2.0)*eps))+1.0)",eps=eps,degree=deg)
    else:
        init_u = Expression("1.0/2.0*(tanh((0.2-sqrt(pow(x[0]-0.3,2)+pow(x[1]-0.5,2)))/(sqrt(2.0)*eps))+1.0) + 1.0/2.0*(tanh((0.2-sqrt(pow(x[0]-0.7,2)+pow(x[1]-0.5,2)))/(sqrt(2.0)*eps))+1.0)",eps=eps,degree=deg)
...
if savepic:
    if stokes:
        plt.savefig("fig/u_FE+Eyre_stokes_nt-%d_t-%.5f_P%d.png" %
                    (num_steps, 0.0, deg))
    else:
        plt.savefig("fig/u_FE+Eyre_nt-%d_t-%.5f_P%d_adv-%.1f_nx-%d.png" %
...
if(savesol):
    ...
    # fFile.close()
    if stokes:
        with XDMFFile("fig/u_sol_FE_stokes_nt-%d_T-%.3f_P%d_counters-5.xdmf" %(num_steps, T, deg)) as outfile:
            # outfile.write(mesh)
            counter  = 0
            outfile.write_checkpoint(u_0, "u", counter, append=False)
    else:
        with XDMFFile("fig/u_sol_FE_nt-%d_T-%.3f_P%d_adv-%.1f_nx-%d_counters-5.xdmf" %(num_steps, T, deg, adv, nx)) as outfile:
            # outfile.write(mesh)
            counter  = 0
            outfile.write_checkpoint(u_0, "u", counter, append=False)
...
a2 = mu_u * baru * dx \
    - pow(eps, 2) * dot(grad(u), grad(baru)) * dx \
    - f_im * baru * dx
L2 = f_ex * baru * dx
...
# Mass lumping
if(mass_lumping):
    u_trial, mu_trial = split(auxtrial)
    auxfunc = Function(W)
    assign(auxfunc, [interpolate(Constant(1.0), V1), interpolate(Constant(0.0), V1)])
    mass_form = u_trial * barmu_u * dx() + mu_trial * baru * dx()
...
for i in range(num_steps):
    ...
    while iteration < iteration_max and convergence != True:
        ...
        J_matrix = assemble(J)
        if(mass_lumping):
            # F_matrix = F_matrix + M_lumped * (solvector_u.vector()-right_vector.vector())
            F_matrix = F_matrix + M_lumped * (solvector.vector()-right_vector.vector())
...
if(savemax and rank==0):
    ...
    if stokes:
        file = open("fig/max_u_FE_stokes_nt-%d_T-%.3f_P%d" %(num_steps, T, deg), "wb")
    else:
        file = open("fig/max_u_FE_nt-%d_T-%.3f_P%d_adv-%.1f_nx-%d" %(num_steps, T, deg, adv, nx), "wb")
    ...
    # open a binary file in write mode
    if stokes:
        file = open("fig/min_u_FE_stokes_nt-%d_T-%.3f_P%d" %(num_steps, T, deg), "wb")
    else:
        file = open("fig/min_u_FE_nt-%d_T-%.3f_P%d_adv-%.1f_nx-%d" %(num_steps, T, deg, adv, nx), "wb")
    # save array to the file
...
if savepic:
    if stokes:
        plt.savefig("fig/FE+Eyre_stokes_nt-%d_t-%.5f_P%d_energia.png" %
                    (num_steps, t, deg))
    else:
        plt.savefig("fig/FE+Eyre_nt-%d_t-%.5f_P%d_adv-%.1f_nx-%d_energia.png" %
                    (num_steps, t, deg, adv, nx))
if showpic:
    plt.show()
plt.close()
...
if(savesol):
    ...
    if(rank==0):
        if stokes:
            file = open("fig/dynamics_FE_stokes_nt-%d_T-%.3f_P%d" %(num_steps, T, deg), "wb")
        else:
            file = open("fig/dynamics_FE_nt-%d_T-%.3f_P%d_adv-%.1f_nx-%d" %(num_steps, T, deg, adv, nx), "wb")
        np.save(file, dynam)
        file.close
    ...
    if(rank==0):
        if stokes:
            file = open("fig/energy_FE_stokes_nt-%d_T-%.3f_P%d" %(num_steps, T, deg), "wb")
        else:
            file = open("fig/energy_FE_nt-%d_T-%.3f_P%d_adv-%.1f_nx-%d" %(num_steps, T, deg, adv, nx), "wb")
        np.save(file, E)
        file.close
