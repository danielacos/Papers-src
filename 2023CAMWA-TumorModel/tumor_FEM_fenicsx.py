...
class Tumor_DG_UPW(object):
    ...
    def __init__(self, tumor_parameters):
        ...
        tumor.Wh = FunctionSpace(mesh, MixedElement([tumor.P1c, tumor.P1c, tumor.P1c]))
        ...
        #
        u, muu, n = tumor.u, tumor.muu, tumor.n = split(tumor.solvector)
        #
        # Test variables
        #
        ub, muub, nb = tumor.ub, tumor.muub, tumor.nb = split(tumor.testvector)
    ...
    def load_initial_values(self):
        ...
        n_init = Expression(n0_dict[p.initial_cond], tumor.Wh.sub(2).element.interpolation_points())
        ...
        tumor.solvector0.sub(2).interpolate(n_init)
        tumor.solvector0.x.scatter_forward()
        n0 = tumor.solvector0.sub(2)

        dF_u = lambda u: 2 * 0.25 * (u * pow(u - 1, 2) + pow(u, 2) * (u - 1))
        tumor.solvector0.sub(1).x.array[tumor.maps[1]] = tumor.project(dF_u(u0) - pow(tumor.eps, 2) * div(grad(u0)) - tumor.chi0 * n0, tumor.spaces[1]).x.array
        tumor.solvector0.x.scatter_forward()
        muu0 = tumor.solvector0.sub(1)

        tumor.u0, tumor.muu0, tumor.n0 = split(tumor.solvector0)
    ...
    def variational_problem(self):
        ...
        u0, muu0, n0 = tumor.u0, tumor.muu0, tumor.n0
        ...
        # PDE functions
        #
        tumor.F_u = lambda u: 0.25 * (pow(u,2)) * (pow(u-1,2))
        f_im = tumor.f_im = 3.0 * 0.25 * u
        f_ex = tumor.f_ex = 0.25 * (4.0*(pow(u0,3)) - 6.0*(pow(u0,2)) - u0)
        ...
        a1 = inner(u, ub) * dx \
            + dt * Cu * Mpos(u) * inner(grad(muu), grad(ub)) * dx \
            - dt * delta * P0 * inner(P(u) * pos(n) * pos(mun(n, u0) - muu), ub) * dx
        L1 = inner(u0, ub) * dx

        a2 = inner(muu, muub) * dx \
            - pow(eps, 2) * inner(grad(u), grad(muub)) * dx \
             - inner(f_im, muub) * dx \
             + chi0 * inner(n, muub) * dx
        L2 = inner(f_ex, muub) * dx

        a3 = tau * inner(n, nb) * dx \
            + dt * Cn * Mpos(n) * inner(grad(mun(n, u0)), grad(nb)) * dx \
            + dt * delta * P0 * inner(P(u) * pos(n) * pos(mun(n, u0) - muu), nb) * dx
        L3 = tau * inner(n0, nb) * dx
    ...
    def time_iterator(self, tsteps=1, first_step=1, verbosity=0):
        ...
        while step < last_step:
            if step == first_step - 1:
                u0, muu0, n0 = tumor.solvector0.split()
                # --- Yield initial data
                yield {'step': step, 't': tumor.t, 'u': u0, 'muu': muu0, 'n': n0, 'mun': tumor.mun(n0,u0)}
...
# -
                ...
                u, muu, n = tumor.solvector.split()
                u0, muu0, n0 = tumor.solvector0.split()

                # --- Yield data computed in current iteration
                yield {'step': step, 't': tumor.t, 'u': u, 'muu': muu, 'n': n, 'mun':mun, 'u0': u0, 'muu0': muu0, 'n0': n0}
...
def print_info(i, t, u_data, n_data, energy, dynamics = 0):
    u_max, u_min, u_n_mass = u_data
    n_max, n_min = n_data
    s = f"{i:3} {t:.6e} {u_max:.4e} {n_max:.4e}"
    s += f" {u_min:.4e} {n_min:.4e}"
    s += f" {u_n_mass:.4e}"
    s += f" {energy:.4e}"
    if dynamics:
        dynamics_u, dynamics_n = dynamics
        s += f" {dynamics_u:.4e} {dynamics_n:.4e}"
    printMPI(s)
...
def define_parameters():
    ...
    parser.add_argument('--vtkfile', default="tumor_FEM", help="Name of vtk file")
    parser.add_argument('--save', default=0, help="No figures and output saved")
    parser.add_argument('--savefile', default="tumor_FEM", help="Name of output file")
...
if(__name__ == "__main__"):
    ...
    # Print info
    #
    printMPI("Time steps:\n  i t u_max n_max u_min n_min u_n_mass energy dynam_u dynam_n")
    ...
    for t_step in tumor_iterations:
        ...
        u, n = t_step['u'], t_step['n']
        muu, mun = t_step['muu'], t_step['mun']
        ...
        n_max, n_min = max(n.x.array[tumor.maps[2]]), min(n.x.array[tumor.maps[2]])
        u_n_mass = assemble_scalar(form((u + n)*dx))
        energy = assemble_scalar(form(
            0.5 * pow(tumor.eps,2) * inner(grad(u), grad(u)) * dx \
            + tumor.F_u(u) * dx \
            - tumor.chi0 * u * n * dx \
        ...
        if t>DOLFIN_EPS:
            u0, n0 = t_step['u0'], t_step['n0']

            dynamics_u = np.abs(u.x.array[tumor.maps[0]]-u0.x.array[tumor.maps[0]]).max()/np.abs(u0.x.array[tumor.maps[0]]).max()
            dynamics_n = np.abs(n.x.array[tumor.maps[2]]-n0.x.array[tumor.maps[2]]).max()/np.abs(n0.x.array[tumor.maps[2]]).max()
        ...
        if (do_plot and i % int(p.tsteps) % int(p.plot) == 0):  # Plot some steps
            ...
            cells, types, x = plot.create_vtk_mesh(tumor.spaces[0])
            grid = pyvista.UnstructuredGrid(cells, types, x)
            aux = u.x.array[tumor.maps[0]]
            aux[np.abs(aux) < 1e-16] = 0
            aux[np.abs(aux - 1.0) < 1e-16] = 1.0
            grid.point_data["u"] = aux
            ...
            if pyvista.OFF_SCREEN:
                figure = plotter.screenshot(f"./{base_name_save}_u_i-{i}.png", transparent_background=True)
                
                img = Image.open(f"./{base_name_save}_u_i-{i}.png")
                width, height = img.size
                # Setting the points for cropped image
                left = width/6
                top = 0.08 * height
                ...
                bottom = 0.92 * height
                im_cropped = img.crop((left, top, right, bottom)) # default window size is 1024x768
                im_cropped.save(f"./{base_name_save}_u_i-{i}.png")
            else:
                plotter.show()
            plotter.close()

            aux = n.x.array[tumor.maps[2]]
            aux[np.abs(aux) < 1e-16] = 0
            aux[np.abs(aux - 1.0) < 1e-16] = 1.0
            grid.point_data["n"] = aux
            ...
            # otherwise create interactive plot
            if pyvista.OFF_SCREEN:
                figure = plotter.screenshot(f"./{base_name_save}_n_i-{i}.png", transparent_background=True)

                img = Image.open(f"./{base_name_save}_n_i-{i}.png")
                width, height = img.size
                # Setting the points for cropped image
                left = width/6
                top = 0.08 * height
                ...
                bottom = 0.92 * height
                im_cropped = img.crop((left, top, right, bottom)) # default window size is 1024x768
                im_cropped.save(f"./{base_name_save}_n_i-{i}.png")
            else:
                plotter.show()
            plotter.close()

            dif_pot = tumor.project(n * tumor.pos(mun - muu), tumor.Vh1)
            grid.point_data["dif_pot"] = dif_pot.x.array[:]
            grid.set_active_scalars("dif_pot")
    ...
    if do_plot:
        ...
        plt.plot(np.linspace(0, t, int(p.tsteps)), dynam_u_list, color='darkblue')
